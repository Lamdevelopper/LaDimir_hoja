...
const state = { document: null, viewport: null, tool: 'pan', lineKind: 'line', pointer: null, lineSelection: [], dirty: false, renderQueued: false, spaceDown: false };
...
function queueRender() {
  ...
  requestAnimationFrame(() => { state.renderQueued = false; const rect = canvas.getBoundingClientRect(); try {
    ...
    if (fn) fn(ctx, state.document, state.viewport, { width: rect.width, height: rect.height, colors: { outside: '#f4f4f4', paper: '#ffffff', millimeter: 'rgba(80,80,80,.12)', centimeter: 'rgba(55,55,55,.34)', border: '#777', point: '#17324d', line: '#1f5aa6', segment: '#13795b', bestFit: '#a33a75' } }); else renderFallback();
...
function activateTool(tool) { state.tool = tool; document.querySelectorAll('.tool-button').forEach((button) => { const active = button.dataset.tool === tool; button.classList.toggle('is-active', active); button.setAttribute('aria-pressed', String(active)); }); canvas.classList.toggle('is-drawing', tool !== 'pan'); canvas.classList.toggle('is-delete', tool === 'delete-line'); if (tool !== 'line') { state.lineSelection = []; $('cancel-line').hidden = true; $('line-help').textContent = 'Activa Recta y selecciona dos puntos.'; } }
...
function nearestPoint(position) { let best = null; let distance = Infinity; for (const p of pointsOf(state.document)) { const q = toScreen(p); const d = Math.hypot(q.x - position.x, q.y - position.y); if (d < distance) { distance = d; best = p; } } return distance <= 24 ? best : null; }
function distanceToSegment(point, start, end) { const dx = end.x - start.x; const dy = end.y - start.y; const lengthSquared = dx * dx + dy * dy; if (!lengthSquared) return Math.hypot(point.x - start.x, point.y - start.y); const t = Math.max(0, Math.min(1, ((point.x - start.x) * dx + (point.y - start.y) * dy) / lengthSquared)); return Math.hypot(point.x - (start.x + t * dx), point.y - (start.y + t * dy)); }
function distanceToInfiniteLine(point, start, end) { const dx = end.x - start.x; const dy = end.y - start.y; const length = Math.hypot(dx, dy); return length ? Math.abs(dy * point.x - dx * point.y + end.x * start.y - end.y * start.x) / length : Infinity; }
function strokeDistance(position, stroke, byId, sheet) { const kind = stroke.kind || 'segment'; if (kind === 'best-fit') { const eq = stroke.equation; if (eq?.axis === 'x') { const a = toScreen({ x: eq.constant, y: 0 }); const b = toScreen({ x: eq.constant, y: sheet.heightCm }); return distanceToSegment(position, a, b); } if (eq?.axis === 'y') { const a = toScreen({ x: 0, y: eq.slope * 0 + eq.intercept }); const b = toScreen({ x: sheet.widthCm, y: eq.slope * sheet.widthCm + eq.intercept }); return distanceToInfiniteLine(position, a, b); } return Infinity; } const from = byId.get(stroke.from); const to = byId.get(stroke.to); if (!from || !to) return Infinity; const a = toScreen(from); const b = toScreen(to); return kind === 'line' ? distanceToInfiniteLine(position, a, b) : distanceToSegment(position, a, b); }
function nearestStroke(position) { if (!state.document) return null; const docSheet = sheetOf(state.document); const docPoint = toDocument(position); if (docPoint.x < 0 || docPoint.y < 0 || docPoint.x > Number(docSheet.widthCm) || docPoint.y > Number(docSheet.heightCm)) return null; const byId = new Map(pointsOf(state.document).map((point) => [point.id, point])); let best = null; let bestDistance = Infinity; const lines = linesOf(state.document); for (let index = lines.length - 1; index >= 0; index -= 1) { const distance = strokeDistance(position, lines[index], byId, docSheet); if (distance < bestDistance) { bestDistance = distance; best = lines[index]; } } return bestDistance <= 14 ? best : null; }
function selectLinePoint(point) { if (!point) { setStatus('Haz clic dentro del área de selección del punto (24 px).', true); return; } if (state.lineSelection.some((p) => p.id === point.id)) return; state.lineSelection.push(point); $('line-help').textContent = `Seleccionados ${state.lineSelection.length}/2 puntos (${state.lineKind === 'segment' ? 'segmento' : 'recta'}).`; $('cancel-line').hidden = false; if (state.lineSelection.length === 2) { const [a, b] = state.lineSelection; const id = `l${Date.now().toString(36)}${linesOf(state.document).length}`; try { const next = callMutation(['addLine', 'insertLine'], [state.document, { id, kind: state.lineKind, from: a.id, to: b.id }]); if (next?.points) state.document = next; else if (!next && !linesOf(state.document).some((item) => item.id === id)) state.document.lines.push({ id, kind: state.lineKind, from: a.id, to: b.id }); replaceDocument(state.document); setStatus(state.lineKind === 'segment' ? 'Segmento creado entre los dos puntos.' : 'Recta creada a partir de los dos puntos.'); } catch (error) { setStatus(`No se pudo crear el trazo: ${error.message}`, true); } state.lineSelection = []; $('cancel-line').hidden = true; $('line-help').textContent = 'Activa Recta y selecciona dos puntos.'; } }
function addBestFitLine() { const fn = Model.addBestFitLine ?? Model.createBestFitLine; if (typeof fn !== 'function') { setStatus('La recta automática aún no está disponible en este modelo.', true); return; } if (pointsOf(state.document).length < 2) { setStatus('La recta automática necesita al menos dos puntos.', true); return; } try { const result = fn(state.document); if (result?.points) state.document = result; replaceDocument(state.document); setStatus('Recta automática calculada con todos los puntos.'); } catch (error) { setStatus(`No se pudo calcular la recta automática: ${error.message}`, true); } }
function deleteStroke(stroke) { if (!stroke) { setStatus('Haz clic cerca de una recta o segmento.', true); return; } try { const remove = Model.removeLine; if (typeof remove !== 'function') throw new Error('La eliminación de trazos no está disponible.'); remove(state.document, stroke.id); replaceDocument(state.document); setStatus('Trazo borrado.'); activateTool('pan'); } catch (error) { setStatus(`No se pudo borrar el trazo: ${error.message}`, true); } }
...
function createFromForm(event) { event.preventDefault(); const form = new FormData($('sheet-form')); const name = String(form.get('name') || '').trim(); const widthCm = Number(form.get('widthCm')); const heightCm = Number(form.get('heightCm')); if (!name || !Number.isFinite(widthCm) || !Number.isFinite(heightCm) || widthCm < 1 || heightCm < 1 || widthCm > 500 || heightCm > 500) { setStatus('Indica un nombre y tamaños entre 1 y 500 cm.', true); return; } state.document = createDocument({ name, widthCm, heightCm }); state.viewport = makeViewport(); state.lineSelection = []; state.lineKind = 'line'; $('line-kind').value = 'line'; state.dirty = false; updateInfo(); queueRender(); $('sheet-dialog').close?.(); activateTool('pan'); setStatus(`Nueva hoja creada: ${name}.`); }

canvas.addEventListener('pointermove', (event) => { const p = screenPoint(event); updateCoordinates(p); if (state.tool === 'line' && state.document) { const hit = nearestPoint(p); canvas.classList.toggle('has-hit-target', Boolean(hit)); canvas.title = hit ? `Seleccionar punto ${hit.label || hit.id}` : 'Selecciona un punto'; } else if (state.tool === 'delete-line' && state.document) { const hit = nearestStroke(p); canvas.classList.toggle('has-hit-target', Boolean(hit)); canvas.title = hit ? `Borrar ${hit.kind === 'segment' ? 'segmento' : 'recta'}` : 'Selecciona un trazo'; } else { canvas.classList.remove('has-hit-target'); canvas.removeAttribute('title'); } if (state.pointer?.id === event.pointerId) { const dx = p.x - state.pointer.last.x; const dy = p.y - state.pointer.last.y; if (Math.hypot(p.x - state.pointer.start.x, p.y - state.pointer.start.y) > 3) state.pointer.dragging = true; if (state.tool === 'pan' || state.pointer.button === 1 || state.pointer.space) state.viewport?.panBy?.(dx, dy); state.pointer.last = p; queueRender(); } });
...
canvas.addEventListener('pointerup', (event) => { const p = screenPoint(event); const active = state.pointer; state.pointer = null; canvas.releasePointerCapture?.(event.pointerId); if (!active || active.dragging) return; if (state.tool === 'point') addPointAt(toDocument(p)); else if (state.tool === 'line') selectLinePoint(nearestPoint(p)); else if (state.tool === 'delete-line') deleteStroke(nearestStroke(p)); });
canvas.addEventListener('pointercancel', () => { state.pointer = null; });
canvas.addEventListener('pointerleave', () => { canvas.classList.remove('has-hit-target'); if (state.tool === 'line') canvas.removeAttribute('title'); });
...
$('line-kind').addEventListener('change', (event) => { state.lineKind = event.target.value === 'segment' ? 'segment' : 'line'; $('line-help').textContent = `Modo activo: ${state.lineKind === 'segment' ? 'segmento' : 'recta infinita'}. Selecciona dos puntos.`; });
$('best-fit').addEventListener('click', addBestFitLine);
